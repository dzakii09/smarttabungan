...
class PaymentGatewayService {
  ...
  async setupRecurringPayment(
    ...
    try {
      // Validate payment method
      const method = this.supportedPaymentMethods.find(
        m => m.id === paymentMethod && m.isActive
      );
      ...
      throw new Error(`Gagal mengatur pembayaran berulang: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  // Cancel recurring payment
  async cancelRecurringPayment(recurringId: string, userId: string) {
    try {
      const recurringPayment = await prisma.recurringTransaction.findFirst({
        where: { id: recurringId, userId }
      });

      if (!recurringPayment) {
        throw new Error('Pembayaran berulang tidak ditemukan');
      }
      ...
      return {
        success: true,
        message: 'Pembayaran berulang berhasil dibatalkan'
      };
    } catch (error) {
      console.error('Error canceling recurring payment:', error);
      throw new Error('Gagal membatalkan pembayaran berulang');
    }
  }
