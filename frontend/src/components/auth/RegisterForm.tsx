...
const RegisterForm: React.FC<RegisterFormProps> = ({ onShowLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await api.post<{ token: string; user: any }>('/auth/register', { name, email, password });
      ...
      setIsLoading(false);
      setError(
        err.response?.data?.message || 'Registrasi gagal. Silakan cek data Anda.'
      );
    }
  };

  return (
    <div className="w-full min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-50 to-secondary-100 transition-all duration-500">
      <div className="flex flex-col md:flex-row bg-white rounded-3xl shadow-2xl overflow-hidden animate-fade-in-up">
        ...
        <div className="w-full max-w-md p-8 md:p-12 flex flex-col justify-center">
          <div className="text-center mb-8">
            ...
            <h2 className="text-3xl font-extrabold text-neutral-800 font-inter mb-1 animate-fade-in">Daftar Akun SmartWealth</h2>
            <p className="text-neutral-500 mt-1 animate-fade-in delay-100">Mulai kelola keuangan Anda dengan lebih pintar</p>
          </div>
          ...
          <div className="mt-8 text-center animate-fade-in-up delay-300">
            <p className="text-neutral-600">
              Sudah punya akun?{' '}
              <button className="text-primary-500 hover:text-primary-700 font-bold underline underline-offset-2 transition-colors" onClick={onShowLogin}>Masuk</button>
            </p>
          </div>
        </div>
      </div>
